...
//			pst = db.get().prepareStatement("SELECT * FROM manufecturear WHERE username=? AND password=?");
			pst = db.get().prepareStatement(
					"SELECT * FROM manufecturear WHERE username=? AND password=? AND status='Active'");
	...
	public void signupManufecturear(ManufecturearSignup mUp) {
		try {
			pst = db.get().prepareStatement(
					"INSERT INTO manufecturear(manu_name, product_type, website, email, rep_name, rep_deg, rep_email, rep_phone, "
	...
	public void addProduct(Product pdt) {
		try {
			pst = db.get().prepareStatement(
					"INSERT INTO products(p_id, manu_id, p_name, p_generic, price, stock, picture, p_description, p_status) VALUES(?, ?, ?, ?, ?, ?, ?, ?, ?)");
	...
	public List<Product> showProduct() {
		...
		try {
			...
			ResultSet rs = pst.executeQuery();
			while (rs.next()) {
				pdt = new Product(rs.getString(1), rs.getString(2), rs.getString(3), rs.getString(4), rs.getDouble(5),
	...
	public void update(Product pdt) {
		try {
			pst = db.get().prepareStatement(
					"UPDATE products SET manu_id = ?, p_name = ?, p_generic = ?, price = ?, stock = ?, picture = ?, p_description = ?, p_status = ? WHERE p_id = ?");
	...
	public List<Order> getOrderInfoByManufacturerId(int id) {
		...
		Order odr;
		try {
			pst = db.get().prepareStatement("SELECT * FROM orders WHERE NOT status='Pending' AND manu_id = ?");
			pst.setInt(1, id);
			ResultSet rs = pst.executeQuery();
			while (rs.next()) {
				odr = new Order();
